...
<PRE>
mkdir /tmp/findmyso_example
echo "Dummy .so" >> /tmp/findmyso_example/libutil.so
gcc -o main main.c findmyso.c  -Wl,-rpath,/tmp/findmyso_example
./main libutil.so:libcurl.so
</PRE>
The result might look like this:
<PRE>
/* findmyso version 0.9.0 */
/* LD_PRELOAD */
/* DT_RPATH */
/* LD_LIBRARY_PATH */
...
findmyso.c, a C program with a findmyso() routine that's ordered to "find my .so" files.<BR>
findmyso.h, a small file to include in any program that calls findmyso<BR>
main.c, an example program that includes findmyso.h and calls the findmyso() routine<BR>
findmyso_tests.sh, a script that checks the assumptions and claims made about .so searching<BR>
README.md, this file.</P>

<P>There is one callable function in findmyso.c, named findmyso.
...
<P>The assumed environment is: linux containing gcc and the almost-always-present utilities ldconfig, uname.</P>

<H3 id="Re Errors">Re Errors</H3><HR>
  As well as filling the buffer, findmyso returns an error code as defined in findmyso.h:
  0 FINDMYSO_OK no error
  -1 FINDMYSO_ERROR_MAX_BUFFER_LENGTH_TOO_SMALL execution stopped after output of nearly buffer_max_length bytes
  -2 FINDMYSO_ERROR_NULL e.g. findmyso() was called with a null pointer so nothing was done
  -3 FINDMYSO_ERROR_MAX_PATH_LENGTH_TOO_SMALL e.g. a path is 5000 bytes (the default maximum is 4096)
  ... In fact anything other than 0 should be extremely rare.

<H3 id="Re Filter">Re Filter</H3><HR>
  From the passed sonames, we use findmyso_find_line_in_sonames() to filter the results of readdir() + popen("/sbin/ldconfig").
  ...
  So we'd call findmyso("libmysqlclient.so:libmariadb.so:libmariadbclient.so:tarantool.so", ...);
...
elf.h seems to be standard but it might be useless to try to look for it with FreeBSD or if mingw.
...
<P>Delivers the result of ld_config -p
If /sbin/ldconfig is not found (very unlikely), there will be will be no error but no reading of the cache.
This is one of the things that seems to be Linux only.</P>
...
Example: if the executable is /home/pgulutzan/findmyso/main libcrypto,
         and there is a file libcrypto.so on /home/pgulutzan/findmyso/lib,
         and the gcc has -Wl,-rpath,\$ORIGIN/lib (but good luck trying to specify it in a script or cmake!),
         then the display will be
         /home/pgulutzan/findmyso/lib/libcrypto.so
         and not $ORIGIN/lib/libcrypto.so, but the comment "/* $ORIGIN was replaced */" may also appear.
...
<P>
It sees and replaces these according to what the Linux documentation says.
...
FreeBSD doesn't have ldconfig -p, at least it's not documented, so expect no output via ld.so.cache.
...
For example, the user might not have permission.</P>
...
<H3 id="Re FINDMYSO_MAX_PATH_LENGTH">Re FINDMYSO_MAX_PATH_LENGTH</H3><HR>
<P>See https://stackoverflow.com/questions/833291/is-there-an-equivalent-to-winapis-max-path-under-linux-unix
Conclusion: depending on #include linux/limits.h won't do. So there's a #define FINDMYSO_MAX_PATH_LENGTH 4096.
...
Emphasis: the loader prefers to grab the exact soname e.g. libcrypto.so.1.1. The fact that you get hits for libcrypto.so is just a hint.</P>

<H3 id="Re findmyso_tests.sh">Re findmyso_tests.sh</H3><HR>
...
say chmod +x then ./findmyso_tests.sh
...
<PRE>
gcc -Wall -o main main.c findmyso.c  -Wl,-rpath,./lib,-rpath,"/tmp/A B"
./main libcrypto.so
export LD_LIBRARY_PATH=/usr/lib64/'unusual path'
./main libcrypto.so
gcc -o main main.c findmyso.c -Wl,-rpath,./lib,-rpath,./ocelotgui,-disable-new-dtag
./main libcrypto.so
gcc -o main main.c findmyso.c -Wl,-z,origin,-rpath,./lib,-rpath,\$ORIGIN
./main libcrypto.so
gcc -o main main.c findmyso.c -DFINDMYSO_INCLUDE_COMMENTS=1 -Wl,-z,origin,-rpath,./lib,-rpath,\$ORIGIN
